...
class MoviesModProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = MoviesModParser.parseCards(text, url).map { card ->
            if (card.isSeries) {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val text = app.get(url).text
        return MoviesModParser.parseCards(text, url).map { card ->
            if (card.isSeries) {
                newTvSeriesSearchResponse(card.title, card.url, TvType.TvSeries) {
                    this.posterUrl = card.poster
                }
    ...
    private suspend fun expandArchive(archiveUrl: String): List<Pair<String, String>> {
        repeat(3) { attempt ->
            try {
                ...
                val out = MoviesModParser.parseArchiveLinks(res.text, res.url)
                if (out.isNotEmpty()) {
    ...
    override suspend fun load(url: String): LoadResponse {
        val res = app.get(url)
        val detail = MoviesModParser.parseDetail(res.text, res.url)
        ...
        val groups = MoviesModParser.buildQualityGroups(detail, ::expandArchive)

        return if (detail.isSeries) {
            val usable = groups.filter { it.episodes.isNotEmpty() }
            val plans = MoviesModParser.buildEpisodePlans(detail.title, usable, detail.poster)
            diag(
                "load-series",
            ...
            val data = MoviesModParser.moviePayloadData(url, sources)
    ...
    override suspend fun loadLinks(
        ...
        val labels = anchors.mapNotNull {
            val href = MoviesModParser.resolveUrl(docUrl, it.attr("href").trim())
            val label = it.text().trim()
            if (href.contains("cloud.unblockedgames.world") && label.isNotBlank()) {
                href to label
            } else null
        }.toMap()
        val targets = anchors.map { MoviesModParser.resolveUrl(docUrl, it.attr("href").trim()) }
            .filter { href ->
    ...
    companion object {
        private const val TAG = "MoviesMod"
    }
}
